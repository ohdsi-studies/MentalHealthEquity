...
You will need to have Julia installed on your computer. The Julia version must be greater than or equal to version 1.7. Furthermore, the Julia executable must be available from the system `PATH` or you can set the `JULIA_BINDER` R environmental variable to where the Julia `bin`directory is on your computer like this: 
...
To install Julia packages and activate our analysis environment, we can run the following code:
...
Then, from within RStudio, navigate to the folder `MentalHealthEquity/study`.
